...
use business\models\Order;
use Yii;
use common\models\Func;
use business\models\MemberDeposit;
...
class OrderController extends BaseController
{
    ...
    public function actionManageMent(){
        ...
        $query = Order::find()->andWhere(['between', 'order_time', $start, $end])->andWhere(['<>','status',1]);
        if (!$venue_id){
    ...
    /**
     * @api {get} /business/order/details 订单详情
     * @apiName        2订单详情展示
     * @apiDescription   订单展示
     * @apiGroup       order
    ...
    public function actionRefundApproval(){
        ...
        $query = Order::find()->alias('o')->select('o.id,o.card_name,o.total_price,o.member_name as name,o.apply_time,ee.name as names,o.create_id,o.note,o.status,o.refund_note,o.refuse_note')->joinWith('employee ee');
        if ($type == 1) $query->where(['o.status'=>4]);
        if($type==2)$query->where(['o.status'=>['5,6']]);
        if ($keyword) $query->andWhere(['o.status'=>['4,5,6']])->andWhere(['or', ['like', 'o.id', $keyword], ['like', 'ee.name', $keyword]]);
    ...
    public function actionApprovalDetails($orderid){
        $query = Order::find()
            ->alias('o')
            ->select('o.payee_name,o.id,o.order_number,o.card_name,o.venue_id,o.total_price,o.apply_time,o.member_name as name,o.create_id,o.note,o.status,o.refund_note,o.refuse_note,org.name as venue_name,o.member_id,md.pic')
            ->joinWith('memberDetails md')
            ->joinWith('organization org')->where(['o.id'=>$orderid,'venue_id'=>Yii::$app->params['authVenueIds']])
            ->asArray()
            ->all();
        return $query;
    }
    ...
    public function actionRefundApprovalApply(){
        $post  = \Yii::$app->request->post();
        $order =Order::findOne($post['orderid']);
        if (isset($order) && $order->status!==4){
            $this->error('该订单已更改！');
        }
        if (!isset($post['orderid'])) return $this->error('订单id必填项！');
        if ($post['status']==2){
            if (!empty($order)){
                $order->create_id   = isset(\Yii::$app->user->identity->id) ?\Yii::$app->user->identity->id:0;    //操作人
