...
use service\models\MemberClass;
...
class MemberController extends BaseController
{
    ...
    /**
     * @api {get} /serice/member/list  会员列表
     * @apiName        1会员列表
     * @apiGroup       member
...
//            echo 1;
            $query = Member::find()->alias('m')
                ...
                ->joinWith(['memberCard mc' => function($query){
                    $query->joinWith('bindPack bp', FALSE);
                }])
                ->where(['m.venue_id'=>$this->venueId, 'm.private_id'=>0, 'mco.member_id'=>NULL,'m.member_type'=>1])
                ->andWhere(['in', 'mc.status', [1, 4]])
                ->andWhere(['>', 'mc.invalid_time', time()])
                ->andWhere(['bp.polymorphic_type' => 'hs'])
                ->andWhere(['<>', 'bp.polymorphic_id', 0]);
    ...
    /**
     * @api {get} /service/member/view?accesstoken=666   会员详情
     * @apiVersion  1.0.0
     * @apiName        .会员详情
     * @apiGroup       member
     * @apiPermission 管理员
    ...
    /**
     * @api {get} /service/member/class?accesstoken=666  会员列表带上课信息
     * @apiVersion  1.0.0
     * @apiName        5会员列表带上课信息
     * @apiGroup       member
     * @apiPermission 管理员
