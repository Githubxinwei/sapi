...
{
    /**
     * @api {get} /service/approval/list  卡种审批列表 /消息lists
     * @apiName        1卡种审批列表
     * @apiGroup       category
     ...
     * @apiSampleRequest  http://apiqa.aixingfu.net/service/approvals
     ...
     * @apiSuccessExample {json}返回值详情（成功）
    {
        ...
        "data": {
            ...
            "_links": {
                "self": {
                    "href": "http://127.0.0.3/service/approvals?accesstoken=001_1544587932&type=3&page=1"
                }
            },
            "_meta": {
                "totalCount": 3,
    ...
    public function actionList()
    {
        ...
        if($type!=3) $query->joinWith('approvalDetails ad')->andWhere(['ad.approver_id'=>$this->employeeId,'ad.status'=>$type,'ad.type'=>$arType]);

        $model = $query->joinWith(['approvalType at'=>function($q) use($arType){
    ...
    public function actionLists()
    {
        $keyword = Yii::$app->request->get('keyword', 0);
        $query = Approval::find()->alias('a');
        $query->joinWith('approvalDetails ad')->andWhere(['ad.approver_id'=>$this->employeeId,'ad.status'=>[1],'ad.type'=>1]);

        $query->joinWith(['approvalType at'=>function($q){
            $q->joinWith('approvalRole ar')->where(['ar.type'=>[1], 'ar.employee_id'=>$this->employeeId]);
        }]);
    ...
    /**
     * @api {get} /service/approval/detail  卡种审批详情
     * @apiName        2卡种审批详情
     * @apiGroup       category
     ...
     * <span><strong>邮    箱：</strong></span>wangliangliang@itsports.club
     * <span><strong>创建时间：</strong></span>2018/01/17
     * @apiSampleRequest  http://apiqa.aixingfu.net/service/approval/detail
     * @apiSuccess (返回值) {json} data
     * @apiSuccessExample {json}返回值详情（成功）
    ...
    public function actionDetail(){
        ...
        if (!$id) return $this->error('请选择要查看详情！');
        $model = Approval::findOne($id);
        return $model;
        
    }

    /**
     * @api {POST} /service/approval/details  同意或拒绝审批
     * @apiName        2同意或拒绝审批
     * @apiGroup       category
     ...
     * @apiSampleRequest  http://apiqa.aixingfu.net/service/approval/details
      * @apiSuccess (返回值) {json} data
     * @apiSuccessExample {json}返回值详情（成功）
    {
        "message": "审批成功",
    ...
    public function actionDetails()
    {
        ...
        if($model->status > 1) return $this->error('已审批过了');
        ...
        try{
            ...
            return $this->error(\Qiniu\json_decode($e->getMessage()));
        }
    ...
    /**
     * @api {POST} /service/approval/comment  评论审批
     * @apiName        2评论审批
     * @apiGroup       category
     * @apiParam  {int}                id                    审批ID
     ...
     * @apiSampleRequest  http://apiqa.aixingfu.net/service/approval/comment
      * @apiSuccess (返回值) {json} data
     * @apiSuccessExample {json}返回值详情（成功）
    {
        "message": "评论成功",
